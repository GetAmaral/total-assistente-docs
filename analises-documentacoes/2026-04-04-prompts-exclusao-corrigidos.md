# Prompts de Exclusao Corrigidos — Evento + Financeiro

**Data:** 2026-04-04
**Base:** Testes BATERIA-EDICAO-EXCLUSAO-2026-04-03, estudo BBBK + aviacao, boas praticas OpenAI/Anthropic
**Aplicacao:** Colar no campo `value` do node Set correspondente no N8N

---

## Boas Praticas Aplicadas

| Pratica | Fonte | Como foi aplicada |
|---------|-------|-------------------|
| Papel claro no inicio | OpenAI, Anthropic | Primeira frase define quem e o agente e o que faz |
| Instrucoes positivas ("faca X") em vez de negativas ("nao faca Y") | Anthropic | Reescrito para dizer O QUE fazer, nao so o que evitar |
| Exemplos concretos (few-shot) | OpenAI, Anthropic | Exemplos reais de input/output em cada cenario |
| Separacao clara de secoes | OpenAI | Blocos com titulos explicitos, hierarquia visual |
| Raciocinio em etapas (chain of thought) | OpenAI, Anthropic | Fluxo passo-a-passo obrigatorio com decisoes explicitas |
| Formato de saida rigido com exemplo | OpenAI | JSON exato com exemplo valido logo no inicio |
| Guardrails explicitos | Anthropic | Regras de seguranca com consequencia clara |
| Prioridade explicita entre regras | Anthropic | Numeracao de prioridade quando regras podem conflitar |
| Camadas de defesa (aviacao) | Swiss Cheese, TEM, CRM | Buscar → confirmar → executar → verificar → responder |
| Design backward (BBBK) | Caso BBBK | Prompt projetado a partir da promessa ao usuario |

---

## PROMPT 1 — Exclusao de Eventos da Agenda

**Node:** `prompt_excluir` (Set node no Fix Conflito v2)
**Campo:** `prompt` (assignment value)
...
## PROMPT 2 — Exclusao de Registros Financeiros

**Node:** `excluir2` (Set node no Fix Conflito v2 e User Standard)
**Campo:** `prompt` (assignment value)
...
## PROMPT 3 — Regra de Exclusao do Classificador (Escolher Branch)

**Node:** `Escolher Branch` (chainLlm no Fix Conflito v2)
**Trecho a substituir:** a secao "REGRA DE EXCLUSAO" inteira
...
## Resumo das Diferencas: Atual vs Corrigido

### Prompt de Exclusao de Eventos

| Aspecto | Atual | Corrigido |
|---------|-------|-----------|
| Estrutura | Bloco unico denso | 10 secoes com titulos claros |
| Exemplos | Nenhum input→output completo | 6 exemplos com etapas detalhadas |
| Confirmacao | "1 resultado → exclui direto" | Confirma quando contexto parcial, exclui direto quando match exato |
| Verificacao pos-exclusao | "chamar buscar_eventos novamente" (LLM ignora) | Verificar resposta da tool (mais confiavel) |
| Campo ID | "coloque o id encontrada" (ambiguo) | "campo uuid do resultado de buscar_eventos" (preciso) |
| Recorrentes | Nao mencionado | Secao dedicada com confirmacao |
| Formato de saida | Descrito em texto | Exemplo JSON valido no inicio |
| Respostas | 4 templates | 6 templates (adicionou INCERTO e CANCELADO) |

### Prompt de Exclusao Financeira

| Aspecto | Atual (premium) | Atual (standard) | Corrigido (unificado) |
|---------|----------------|-------------------|----------------------|
| Tamanho | ~30 linhas | ~200 linhas | ~180 linhas (equilibrado) |
| Estrutura | Compacto demais | 10 secoes boas | 10 secoes otimizadas |
| Confirmacao | Nenhuma | Nenhuma | Confirma quando contexto parcial |
| Verificacao | "nunca afirmar sem tool" (regra fraca) | "nunca afirmar sem tool" (regra fraca) | Etapa 5 obrigatoria com 3 cenarios |
| Exemplos | 0 | 0 | 7 exemplos completos |
| Multiplas exclusoes | Mencionado vagamente | Mencionado vagamente | Exemplo concreto (ex 6) |
| Entradas/receitas | Nao diferenciado | Nao diferenciado | "Entrada removida!" (ex 7) |
| Desempate | 3 regras | 3 regras | Mesmas 3 regras + fallback claro |
| "Ultimo gasto" | Regra OK mas sem exemplo | Regra OK mas sem exemplo | Regra + exemplo completo (ex 3) |

### Classificador (Escolher Branch)

| Aspecto | Atual | Corrigido |
|---------|-------|-----------|
| Regra | "verbos EXPLICITOS de exclusao" | "VERBO PRINCIPAL da frase" |
| Teste | Nenhum (keyword matching) | Teste de 2 passos obrigatorio |
| Exemplos | 0 | 8 exemplos (4 criacao com palavra de exclusao no nome, 4 exclusao real) |
| Regra de ouro | Nao existe | "Frase com verbo de criacao + palavra de exclusao no nome = SEMPRE criacao" |
